...
public class DisplayUsers extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // Asynchronous POST
        try {
            listResults = AsyncPost.doAsyncPost("getuserslist", listIPSrv, inputJSON);
        } catch (Exception e) {
        ...
        JsonObject outputJSON = ResponsesManager.mergeMaps(listResults.get(0), listResults.get(1), new String[] {"error"},
                new String[] {"users_list"}, new String[] {}, listIPSrv.size());

        // Bad case
        if (!outputJSON.has("users_list") && !outputJSON.has("error"))
