...
public class DisplayGroups extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // Asynchronous POST
        try {
            listResults = AsyncPost.doAsyncPost("getgroups", listIPSrv, inputJSON);
        } catch (Exception e) {
        ...
        JsonObject outputJSON = ResponsesManager.mergeMaps(listResults.get(0), listResults.get(1), new String[] {"error"},
                new String[] {}, new String[] {"groups"}, listIPSrv.size());


        // Bad case
        if (!outputJSON.has("groups") && !outputJSON.has("error"))
